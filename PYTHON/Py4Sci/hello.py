...
# 实验目的：
&emsp;&emsp;分别使用上文提到的**运筹学(规划论角度)**、**算法设计(图论角度)** 方法求解商品调配问题的最优方案，并分析两种方法的优缺点，完成学科交叉与讨论。同时，为了提高程序的可移植性，也为了彰显本课程强调的$Python$编程能力，**求解程序已制作成网页并上传至云服务器**，以供在线求解问题(网页链接：[*求解器*](https://repo2024-dfiqzv8hgeodcmtvegc6zf.streamlit.app/))。
...
## 图论算法：
&emsp;&emsp;上一小节完全从规划论的角度考虑问题，我们不妨转换思路，从图论的角度重新考虑该问题。
...
&emsp;&emsp;通过商品调配问题的求解，不仅将**运筹学**和**算法设计**这两门课的内容有机地结合起来，而且使得我们能从规划论、图论两个角度看待同一个问题。同时，我在编写程序、设计网页的过程中，也提高了个人的代码能力。我想，这就是**面向科学问题求解的编程实践**这门课想要传递的精神。''')
st.markdown("# 求解器：")
